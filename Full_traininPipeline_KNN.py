...
for i in tqdm(range(10), desc="Runs voltooid"):
    ...
    output_path = f"Dannyfolder/Output/Full_PapagAIo_KNN+pipeline{i+1}.txt"
    ...
    # === Pipeline met BERT + KNN
    pipeline = Pipeline([
        ('bert', BertEmbeddingTransformer(model_name='emilyalsentzer/Bio_ClinicalBERT')),
        ('clf', KNeighborsClassifier(n_neighbors=5, weights='distance'))  # probability=True by default
    ])
    ...
    results = ["=== EVALUATIE OP PAPAGAIO TESTDATA (KNN) ===\n", report, conf_matrix_text, auc_text, "\n"]
    ...
    print(f"✅ Evaluatie Run {i+1} (KNN) opgeslagen in: {output_path}")
