...
for i in tqdm(range(10), desc="Runs voltooid"):
    ...
    output_path = f"Dannyfolder/Output/Full_PapagAIo_NN+pipeline{i+1}.txt"
    ...
    # === Pipeline met BERT + MLPClassifier (Neuraal Netwerk)
    pipeline = Pipeline([
    ...
    results = ["=== EVALUATIE OP PAPAGAIO TESTDATA (Neuraal Netwerk) ===\n", report, conf_matrix_text, auc_text, "\n"]
    ...
    print(f"✅ Evaluatie Run {i+1} (NN pipeline) opgeslagen in: {output_path}")
