...
for i in tqdm(range(10), desc="Runs voltooid"):
    ...
    for ext_label, ext_path in external_test_paths.items():
        ...
        # 4d) Bereken class weights en sample weights
        ...
        # Maak array met sample weights
        sample_weights = np.array([class_weight_dict[label] for label in y_combined_train])

        # 4e) Pipeline bouwen en trainen met een eenvoudige Feedforward NN
        pipeline = Pipeline([
        ...
        # Pas sample_weight toe op de classifier
        pipeline.fit(
            X_combined_train,
            y_combined_train,
