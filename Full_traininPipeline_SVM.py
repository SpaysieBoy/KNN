...
for i in tqdm(range(10), desc="Runs voltooid"):
    ...
    output_path = f"Dannyfolder/Output/Full_PapagAIo_SVM+pipeline{i+1}.txt"
    ...
    # === Pipeline met BERT + SVM
    pipeline = Pipeline([
        ('bert', BertEmbeddingTransformer(model_name='emilyalsentzer/Bio_ClinicalBERT')),
        ('clf', SVC(class_weight=class_weight_dict,
                    C=1.0,
                    probability=True,  # voor predict_proba
                    kernel='linear',
                    random_state=42+i))
    ])
    ...
    results = ["=== EVALUATIE OP PAPAGAIO TESTDATA (SVM) ===\n", report, conf_matrix_text, auc_text, "\n"]
    ...
    print(f"✅ Evaluatie Run {i+1} (SVM) opgeslagen in: {output_path}")
