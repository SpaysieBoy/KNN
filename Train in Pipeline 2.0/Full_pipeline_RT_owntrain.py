...
class BertEmbeddingTransformer(BaseEstimator, TransformerMixin):
    def __init__(self, model_name='emilyalsentzer/Bio_ClinicalBERT', max_length=512):
        ...
        # Verplaats het BERT-model meteen naar het juiste device
    ...
    def transform(self, X):
        ...
        with torch.no_grad():
            for text in X:
                # Tokeniseer en verplaats de input-tensors naar het gekozen device
...
for i in tqdm(range(10), desc="Total runs"):  # i = 0..9
    ...
    for name, path in dataset_paths.items():
        ...
        # Pipeline and train with Decision Tree classifier
        pipeline = Pipeline([
    ...
    # Save per-run file
    save_path = f"Text_File/Train in pipeline 2.0/RT/Full_MultiDataset_RT_Run{i+1}.txt"
...
# Save summary file
summary_path = f"Text_File/Train in pipeline 2.0/RT/Statistical_Summary_RT.txt"
